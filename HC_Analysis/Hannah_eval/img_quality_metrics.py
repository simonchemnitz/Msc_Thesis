...
def Comp_Metrics(all_img, ref, brainmask, whole_image=False, compute_gr=True, normal=True):
    ...
    for i in range(len(all_img)):
        ...
        if compute_gr == True:
            ...
            #ge.append(0)
            tg.append(0)
        coent_lst.append(coent(img, brainmask))
        aes_lst.append(aes(img, brainmask))
    for i in range(len(all_img)):
        print(all_img[i])
        print('PSNR: ', psnr[i])
        print('SSIM: ', ssim[i])
        print('Tenengrad: ', tg[i])
        print("CoEnt: ", coent_lst[i])
        print("AES: " , aes_lst[i])
    ...
    coent_lst, aes_lst = np.array(coent_lst), np.array(aes_lst)
    ...
    coent_lst = coent_lst[ind][::-1]
    aes_lst = aes_lst[ind][::-1]
    names = np.sort(names)[::-1]

    return psnr, ssim, tg, coent_lst, aes_lst, names
